...
- **用量条不要用 `ProgressView`**：macOS 26 真实 `NSPopover` 里 `.tint` 首帧不生效、回落系统 accent 蓝（要等数值变化触发重绘才上色，所以会出现「有的条绿有的条蓝」的灵异现象）；窗口非焦点时控件色还会被灰显改写。列表条已改为自绘 `RemainingBar`（Capsule 轨道 + 按剩余比例的填充，纯形状 + 显式颜色），双端共享（ProviderCard/WindowRow）。
